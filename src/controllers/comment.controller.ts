...
export const getComments = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // ── GLOBAL COMMENT FEED (CHAPTERS ONLY) ───────────────────────────────
    // Only comments that ARE attached to a specific chapter or episode.
    const globalFeed = req.query.global === 'true';
    const chaptersOnly = req.query.chaptersOnly === 'true';
    const source = req.query.source as string | undefined; // anime | manhwa | novel
    if (globalFeed || chaptersOnly) {
      if (source === 'anime') {
        where.animeId = { not: null };
        where.episodeNo = { not: null };
      } else if (source === 'manhwa') {
        where.mangaId = { not: null };
        ...
        where.OR = [
          { AND: [{ animeId: { not: null } }, { episodeNo: { not: null } }] },
          { AND: [{ mangaId: { not: null } }, { chapterId: { not: null } }] },
          { AND: [{ novelId: { not: null } }, { chapterId: { not: null } }] },
        ];
      }
    }
